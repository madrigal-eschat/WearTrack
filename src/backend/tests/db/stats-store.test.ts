...
beforeAll(() => {
  ...
  db.prepare(`INSERT INTO items (category_id, name, color, difficulty_multiplier) VALUES (1,'j','#000',1)`).run();
...
function insertSession(
  ...
  db.prepare(
    `INSERT INTO sessions (id, item_id, started_at, ended_at, target_wear_seconds, max_wear_seconds, rest_seconds)
     VALUES (?, ?, ?, ?, 900, 1800, ?)`,
...
describe('recordCategorySession — streak logic', () => {
  ...
  it('continues streak when gap is within rest_seconds + breakGraceTime', () => {
    ...
    statsStore.recordCategorySession(1, breakGraceTime, {
      id: 100, item_id: 1, started_at: 9100, ended_at: 10000,
      target_wear_seconds: 900, max_wear_seconds: 1800, rest_seconds: restSeconds,
    });

    // Session B starts within (restSeconds + breakGraceTime) = 5400 seconds of prev ended_at
    // starts at 10000 + 5000 = 15000 (gap 5000 < 5400)
    insertSession(101, 1, 15000, 15900, restSeconds);
    statsStore.recordCategorySession(1, breakGraceTime, {
      id: 101, item_id: 1, started_at: 15000, ended_at: 15900,
      target_wear_seconds: 900, max_wear_seconds: 1800, rest_seconds: restSeconds,
    });
  ...
  it('resets streak when gap exceeds rest_seconds + breakGraceTime', () => {
    ...
    statsStore.recordCategorySession(1, breakGraceTime, {
      id: 102, item_id: 1, started_at: 19100, ended_at: 20000,
      target_wear_seconds: 900, max_wear_seconds: 1800, rest_seconds: restSeconds,
    });
    ...
    statsStore.recordCategorySession(1, breakGraceTime, {
      id: 103, item_id: 1, started_at: tooLate, ended_at: tooLate + 600,
      target_wear_seconds: 900, max_wear_seconds: 1800, rest_seconds: restSeconds,
    });
    ...
    expect(afterD.best_streak_count).toBeGreaterThanOrEqual(streakCountBeforeReset);
  });
});

describe('history', () => {
  // Use item_id=2 and session IDs 200+ to avoid cross-test pollution
  it('groups sessions by week and excludes open (null ended_at) sessions', () => {
    ...
    insertSession(200, 2, week1Start, week1Start + 1800, null);     // week 1, 1800s
    insertSession(201, 2, week1Start + 3600, week1Start + 5400, null); // week 1, 1800s
    insertSession(202, 2, week2Start, week2Start + 3600, null);     // week 2, 3600s
    insertSession(203, 2, week2Start + 7200, null, null);           // open — must be excluded
    ...
    expect(rows[0].total_wear_seconds).toBe(3600); // two 1800s sessions in week 1
    expect(rows[0].session_count).toBe(2);
    expect(rows[1].total_wear_seconds).toBe(3600); // one 3600s session in week 2
...
describe('leaderboards', () => {
  it('longestWear returns [] when no sessions have been recorded', () => {
    ...
    categoryStore.create({
      name: 'Empty', icon: 'e',
      initial_target_wear_duration_seconds: 60, initial_max_wear_duration_seconds: null,
      rest_multiplier: 1, minimum_rest: 0,
      risk_levels: [], break_decay_multiplier: 1, break_grace_time: 0,
    });
    // No sessions or stats rows for this category; the query filters WHERE total_wear_seconds > 0
    // For the seeded items that DO have stats, check ordering
    const rows = statsStore.longestWear() as Array<{ max_single_session_wear_seconds: number }>;
  ...
  it('mostTotalWear returns results in descending total_wear_seconds order', () => {
    const rows = statsStore.mostTotalWear() as Array<{ total_wear_seconds: number }>;
    for (let i = 1; i < rows.length; i++) {
      expect(rows[i].total_wear_seconds).toBeLessThanOrEqual(rows[i - 1].total_wear_seconds);
    }
  });
  ...
  it('mostSessions returns results in descending session_count order', () => {
    ...
    for (let i = 1; i < rows.length; i++) {
      expect(rows[i].session_count).toBeLessThanOrEqual(rows[i - 1].session_count);
    }
  });
});
