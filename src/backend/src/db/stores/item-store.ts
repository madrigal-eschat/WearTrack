...
export type ItemUpdate = Partial<Omit<ItemCreate, 'difficulty_multiplier'> & { difficulty_multiplier: number }>;

class ItemStore {
  findAll(categoryId?: number): Item[] {
    if (categoryId !== undefined) {
      return db.prepare('SELECT * FROM items WHERE category_id = ? ORDER BY id').all(categoryId) as Item[];
    }
  ...
  create(data: ItemCreate): Item {
    ...
    const result = db
      .prepare('INSERT INTO items (name, category_id, color, difficulty_multiplier) VALUES (?, ?, ?, ?)')
  ...
  update(id: number, data: ItemUpdate): Item {
    ...
    db.prepare(`UPDATE items SET ${setClauses} WHERE id = ?`).run(...Object.values(data), id);
    return this.find(id)!;
  }
