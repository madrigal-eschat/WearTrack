...
describe('GET /api/sessions/current expected durations', () => {
  it(
    'returns expected_target/expected_max for an idle item (first ' +
      'session)',
    async () => {
  ...
  it(
    ...
    async () => {
      ...
      const cat = await (
        await createCategory({ name: 'Injury Halve Cat' })
      ).json()
      const item = await (
        await createItem(cat.id, { name: 'Injury Shoe', color: '#aabbcc' })
      ).json()
      ...
      const entryBefore = bodyBefore.find(
        (e: { category: { id: number } }) => e.category.id === cat.id,
      )
      const itemBefore = entryBefore.items.find(
        (i: { item_id: number }) => i.item_id === item.id,
      )
      ...
      const entryAfter = bodyAfter.find(
        (e: { category: { id: number } }) => e.category.id === cat.id,
      )
      const itemAfter = entryAfter.items.find(
        (i: { item_id: number }) => i.item_id === item.id,
      )
...
describe('POST /api/sessions/start', () => {
  ...
  it(
    'returns 409 when the category already has an open session on the ' +
      'same item',
    async () => {
  ...
  it(
    'returns 409 when the category already has an open session on a ' +
      'different item',
    async () => {
...
describe('GET /api/sessions/current — items field', () => {
  ...
  it(
    'lists the item with null last-session fields when it ' +
      'has no history',
    async () => {
...
describe('GET /api/sessions/current — decay fields', () => {
  it(
    'returns decay_start_time null and decay_state none when ' +
      'no prior session',
    async () => {
  ...
  it('returns decay_state decaying when past grace period', async () => {
    ...
    const decayingItemRes = await app.request(ITEMS, {
      ...
      body: JSON.stringify({
        name: 'Decaying Shoe',
        category_id: decayingCat.id,
        color: '#ccddee',
      }),
    })
    const decayingItem = await decayingItemRes.json()

    // Under the new floored-loss decay formula, a session that only ever
    // reached
    // the category's initial target (900) decays to the floor in 1 day — too
    // fast
    // to observe a "decaying" (not yet fully decayed) state. Grow the target
    // across
    // three normal rest-and-restart cycles first (900 -> 1800 -> 2700 -> 3600),
    // each
    // restarting exactly at its predecessor's earliest allowed start
    // (ended_at + rest_seconds, with rest_seconds floored to
    // minimum_rest=86400),
    // so every restart takes the normal-growth branch, not decay or halving.
    // The final (4th) session then ends 5 days ago, decays for 3 days since
    // grace
    // (5 days - 2 days of rest+grace), and — per the same worked example as
    // the backend unit tests (calculations.test.ts) — target 3600+900=4500
    // decays
    ...
    const body = await res.json()
    const entry = body.find(
      (e: { category: { id: number } }) => e.category.id === decayingCat.id,
    )
  ...
  it(
    'returns decay_state fully_decayed when target has ' +
      'decayed to initial',
    async () => {
    // Use a fresh category so findLastEndedInCategory only sees this test's
    // session
  ...
  it(
    'rotation categories never show decay, even long after a session ' +
      'ended (would be fully_decayed for a duration category)',
    async () => {
      const now = Math.floor(Date.now() / 1000)
      const rotationCat = await (
        await createCategory({
          name: 'Rotation No Decay',
          type: 'rotation',
      ...
      const rotationItem = await (
        await createItem(rotationCat.id, { name: 'No Decay Shoe' })
      ).json()

      // End a session 10 000 days ago — a duration category would be
      // fully_decayed here.
      const endTs = now - 10_000 * 86400
      const s = await (
        await app.request(`${SESSIONS}/start`, {
      ...
      const body = await res.json()
      const entry = body.find(
        (e: { category: { id: number } }) => e.category.id === rotationCat.id,
      )
...
describe('Stats updates after session end', () => {
  it('increments per-item session_count and total_wear_seconds', async () => {
    const statsBefore = prepare('SELECT * FROM stats WHERE item_id = ?').get(
      itemId,
    ) as {
    ...
    await endSession(s.id)
    const statsAfter = prepare('SELECT * FROM stats WHERE item_id = ?').get(
      itemId,
    ) as {
      session_count: number;
      total_wear_seconds: number;
    ...
    expect(statsAfter.total_wear_seconds).toBeGreaterThan(
      statsBefore.total_wear_seconds,
    )
  })

  it('increments category streak_count after a session', async () => {
    const catStatsBefore = prepare(
...
describe('GET /api/sessions/dates', () => {
  ...
  it('filters by category_id', async () => {
    const res = await app.request(
      `${SESSIONS}/dates?category_id=${categoryId}`,
    )
    expect(res.status).toBe(200)
    const body = await res.json()
    expect(Array.isArray(body)).toBe(true)
...
describe('stats recompute-from-scratch', () => {
  ...
  it(
    ...
    async () => {
      const cat = await (
        await createCategory({ name: 'Recompute Streak Cat' })
      ).json()
      const item = await (
        await createItem(cat.id, { name: 'Recompute Streak Shoe' })
      ).json()
  ...
  it(
    'recomputeItem excludes injury-ended sessions, same as incremental ' +
      'recording',
    async () => {
      const cat = await (
        await createCategory({ name: 'Recompute Injury Cat' })
      ).json()
      const item = await (
        await createItem(cat.id, { name: 'Recompute Injury Shoe' })
      ).json()
      ...
      const stats = prepare('SELECT * FROM stats WHERE item_id = ?').get(
        item.id,
...
describe('PATCH /api/sessions/:id', () => {
  ...
  it('recomputes rest_seconds for the new duration', async () => {
    ...
    // the
    // difference between the two durations exercised below (the default
    // fixture's
    // minimum_rest of 86400 would swallow both).
    const cat = await (
      await createCategory({ name: 'Patch Rest Cat', minimum_rest: 0 })
    ).json()
    const item = await (
      await createItem(cat.id, { name: 'Patch Rest Shoe' })
    ).json()
    const startTs = Math.floor(Date.now() / 1000) - 3600
    const s = await (
      await app.request(`${SESSIONS}/start`, {
  ...
  it('recomputes item and category stats', async () => {
    const cat = await (
      await createCategory({ name: 'Patch Stats Cat' })
    ).json()
    const item = await (
      await createItem(cat.id, { name: 'Patch Stats Shoe' })
    ).json()
    const startTs = Math.floor(Date.now() / 1000) - 3600
    const s = await (
      await app.request(`${SESSIONS}/start`, {
    ...
    const statsBefore = prepare(
      'SELECT total_wear_seconds FROM stats WHERE item_id = ?',
    ).get(item.id) as {
      total_wear_seconds: number;
    }
    ...
    const statsAfter = prepare(
      'SELECT total_wear_seconds FROM stats WHERE item_id = ?',
    ).get(item.id) as {
      total_wear_seconds: number;
    }
    expect(statsAfter.total_wear_seconds).toBe(
      statsBefore.total_wear_seconds - 950,
    )
  })

  it('does not touch stats for an injury-ended session', async () => {
    const cat = await (
      await createCategory({ name: 'Patch Injury Cat' })
    ).json()
    const item = await (
      await createItem(cat.id, { name: 'Patch Injury Shoe' })
    ).json()
    const startTs = Math.floor(Date.now() / 1000) - 3600
    const s = await (
      await app.request(`${SESSIONS}/start`, {
    ...
    const statsBefore = prepare(
      'SELECT session_count FROM stats WHERE item_id = ?',
    ).get(item.id)
...
describe('GET /api/sessions/current streak_count', () => {
  it('is 0 for a fresh category with no sessions', async () => {
    const cat = await (
      await createCategory({ name: 'Streak Fresh Cat' })
    ).json()
  ...
  it('reflects the category streak after consecutive sessions', async () => {
    ...
    const item = await (
      await createItem(cat.id, { name: 'Streak Active Item' })
    ).json()
    ...
    // Second session starts at 100 — within
    // earliest_start(50)..latest_start(50+1000).
    const s2 = await (
      await app.request(`${SESSIONS}/start`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
describe('DELETE /api/sessions/:id', () => {
  ...
  it('recomputes item/category stats after deletion', async () => {
    const cat = await (
      await createCategory({ name: 'Delete Stats Cat' })
    ).json()
    const item = await (
      await createItem(cat.id, { name: 'Delete Stats Shoe' })
    ).json()
    const startTs = Math.floor(Date.now() / 1000) - 3600
    const s = await (
      await app.request(`${SESSIONS}/start`, {
    ...
    const stats = prepare(
      'SELECT session_count, total_wear_seconds FROM stats WHERE item_id = ?',
    ).get(item.id) as { session_count: number; total_wear_seconds: number }
    expect(stats.session_count).toBe(0)
    expect(stats.total_wear_seconds).toBe(0)
  })

  it(
    'removes the session_day_index row when it was the only session for ' +
      'that day',
    async () => {
      const cat = await (
        await createCategory({ name: 'Delete Index Cat' })
      ).json()
      const item = await (
        await createItem(cat.id, { name: 'Delete Index Shoe' })
      ).json()
      const startTs = Math.floor(Date.now() / 1000) - 3600
      const day = new Date(startTs * 1000).toISOString().slice(0, 10)
      const s = await (
...
describe('POST /api/sessions/start — rotation availability', () => {
  it(
    'rejects starting an item that was just worn, before the rest of the ' +
      'rotation has had a turn',
    async () => {
      const cat = await (
        await createCategory({
          name: 'Rotation Sessions',
          type: 'rotation',
          consecutive_wear_days: 1,
          initial_target_wear_duration_seconds: 57600,
          initial_max_wear_duration_seconds: null,
  ...
  it('allows starting an item whose turn it is', async () => {
    ...
    // A's session is on a prior day so it doesn't trip the same-day daily cap
    // for B's attempt
    // below — this test is about rotation-availability, not the daily cap
    // (covered separately).
    ...
    await app.request(`${SESSIONS}/${session1.id}/end`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ended_at: yesterday + 100 }),
    })
...
describe('GET /api/sessions/current — rotation_available', () => {
  it('marks the just-worn item unavailable and others available', async () => {
    ...
    const rowA = entry.items.find(
      (i: { item_id: number }) => i.item_id === itemA.id,
    )
    const rowB = entry.items.find(
      (i: { item_id: number }) => i.item_id === itemB.id,
    )
...
describe('POST /api/sessions/start — consecutive-wear-days lock', () => {
  it(
    'allows re-wearing the same item within the consecutive-wear-days ' +
      'lock, and rejects it once the lock is satisfied',
    async () => {
      const cat = await (
        await createCategory({
          name: 'Consecutive Lock',
          type: 'rotation',
      ...
      // Spread the three wears across three distinct calendar days (>1 day
      // apart each) so the
      // same-day daily cap (a separate, unrelated feature) doesn't interfere
      // with what this test is actually verifying — the fix's OR-eligibility.
      // Wearing the same item on consecutive
      // *days* is exactly the scenario consecutive_wear_days describes anyway.
      const now = Math.floor(Date.now() / 1000)
      const day1 = now - 190000 // ~2.2 days ago
      // ~1 day ago, distinct calendar day from day1 and from today
      const day2 = now - 90000
      ...
      await app.request(`${SESSIONS}/${session1.id}/end`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ended_at: day1 + 100 }),
      })

      // Second (next-day repeat) wear of A: this is the consecutive-lock
      // re-wear the feature
      // exists for. Without the fix, rotationAvailability alone would reject
      // this (A just went,
      // it's B/C's turn per the base rule) — the fix's OR-eligibility must
      // allow it here.
      const start2 = await app.request(`${SESSIONS}/start`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      await app.request(`${SESSIONS}/${session2.id}/end`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ended_at: day2 + 100 }),
      })

      // Third wear of A: consecutive_wear_days (2) is already satisfied by the
      // two prior A sessions.
  ...
  it(
    'leaves items other than the most-recently-worn one, and items whose ' +
      'lock is already satisfied, rejected as before the fix',
    async () => {
      // With 3 items (A, B, C), after only A has worn once, B and C haven't had
      // a turn yet this
      // cycle, so the base rotationAvailability rule (untouched by this fix)
      // already makes them
      // startable — that's correct, unchanged, pre-existing behavior, not
      // something this test
      // needs to re-verify (see the "GET /api/sessions/current —
      // rotation_available" and
      // "POST /api/sessions/start — rotation availability" describe blocks
      // above for that
      // coverage). What this test actually guards against: the new
      // isConsecutiveLockEligible check
      // must not accidentally make an item eligible when it is NOT the
      // most-recently-worn item —
      // i.e. the OR-condition in the controller must be item-identity-scoped,
      // not just
      // category-scoped.
      const cat = await (
        await createCategory({
          name: 'Consecutive Lock Non-Locked Items',
          type: 'rotation',
          consecutive_wear_days: 5,
          initial_target_wear_duration_seconds: 57600,
          initial_max_wear_duration_seconds: null,
        })
      ).json()
      ...
      // A wears, then B wears (both legitimate per the base rule —
      // neither had gone yet). Use
      // explicit, clearly-separated timestamps so findRecentInCategory's `ORDER
      // BY ended_at DESC`
      // has an unambiguous "most recently worn" item — same-second ties would
      // otherwise make
      // ordering between A and B nondeterministic. Both are placed on a prior
      // calendar day so the
      // same-day daily cap (a separate, unrelated feature) doesn't interfere
      // with the startA2/startC
      ...
      await app.request(`${SESSIONS}/${sessionB.id}/end`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ended_at: yesterday }),
      })

      // Now B is the most-recently-worn item. A repeat of A (not the
      // most-recently-worn item, even
      // though A's own run of 1 is well under consecutive_wear_days=5) must
      // still be rejected — the
      // fix's eligibility check is scoped to whichever item was most recently
      // worn, not any item
      ...
      // C, which has never been worn, remains available via the base rule
