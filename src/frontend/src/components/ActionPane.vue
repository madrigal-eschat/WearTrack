<template>
  <div class="action-pane overflow-y-auto">
    ...
    <div
      v-else-if="currentSessions.length === 0"
    ...
    <k-list v-else>
      ...
      >
        <template #title>
          <span v-if="entry.streak_count > 0"
            class="ml-1.5 text-xs text-orange-500 inline-flex items-center
              gap-0.5 align-middle"
            data-testid="streak-badge"
          >
            <Icon icon="ph:flame" class="w-3 h-3" />{{ entry.streak_count }}
          </span>
          <span
            v-if="entry.session && entry.item"
            class="ml-1.5 text-sm font-normal text-gray-500"
        ...
        <template #media>
          <Icon
            ...
            :style="{
              color: entry.session && entry.item
                ? entry.item.color : 'black',
            }"
          />
        ...
        <template #inner>
          <template v-if="entry.session && entry.item">
            <div v-if="isOverdue(entry)"
              class="text-red-600 text-sm font-semibold mt-0.5"
            >Stop wearing</div>
            <WearProgressBar
              class="mt-1"
            ...
            />
            <div class="flex flex-wrap gap-x-4 gap-y-0.5 mt-1 text-sm
              tabular-nums">
              <span class="text-gray-600"
              ><span class="text-xs text-gray-400 uppercase tracking-wide
                mr-1"
              >Worn</span>{{ elapsed(entry.session) }}</span>
              <span
                :class="isOverdue(entry)
                  ? 'text-red-600 font-semibold' : 'text-gray-600'"
              ><span class="text-xs text-gray-400 uppercase tracking-wide
                mr-1"
              >Remaining</span>{{ remainingLabel(entry) }}</span>
              <span class="text-gray-600"
              ><span class="text-xs text-gray-400 uppercase tracking-wide
                mr-1"
              >Target</span>{{ targetLabel(entry) }}</span>
              <span
                v-if="entry.session.max_wear_seconds !== null"
                class="text-gray-600"
              ><span class="text-xs text-gray-400 uppercase tracking-wide
                mr-1"
              >Max</span>{{ maxWear(entry) }}</span>
            </div>
          </template>
          <template v-else>
            ...
            <template v-if="effectiveRestRemainingSeconds(entry) > 0">
              ...
              <WearProgressBar
                mode="rest"
                :fill-fraction="effectiveRestFillFraction(entry)"
              />
            </template>
            <template v-else-if="entry.decay_state !== 'none'">
              ...
              <WearProgressBar
                mode="decay"
                :fill-fraction="decayFillFractionFor(entry)"
              />
              <div class="text-sm font-bold text-black mt-0.5">
                {{ entry.decay_state === 'fully_decayed'
                  ? 'Target and max have fully decayed'
                  : `Total decay in ${decayTimeLeftLabel(entry)}` }}
              </div>
            </template>
            <template v-else>
              <div class="text-xs text-gray-500 min-h-[22px] flex items-center">
                <span v-if="entry.decay_start_time !== null"
                ><span class="text-xs text-gray-400 uppercase tracking-wide
                  mr-1"
                >Start before</span>{{
                  formatDecayDate(entry.decay_start_time)
                }}</span>
            ...
            <div v-if="effectiveRestRemainingSeconds(entry) > 0"
              class="flex flex-wrap gap-x-4 gap-y-0.5 mt-1 text-sm
                tabular-nums">
              <span class="text-gray-600"
              ><span class="text-xs text-gray-400 uppercase tracking-wide
                mr-1"
              >Remaining</span>{{
                shortDuration(effectiveRestRemainingSeconds(entry))
              }}</span>
              <span class="text-gray-600"
              ><span class="text-xs text-gray-400 uppercase tracking-wide
                mr-1"
              >Total</span>{{
                shortDuration(effectiveRestTotalSeconds(entry))
              }}</span>
            </div>
            <div v-else-if="selectedItemData(entry)"
              class="flex flex-wrap gap-x-4 gap-y-0.5 mt-1 text-sm
                tabular-nums">
              <span class="text-gray-600"
              ><span class="text-xs text-gray-400 uppercase tracking-wide
                mr-1"
              >Target</span>{{ idleTarget(entry) }}</span>
              <span
                v-if="idleMax(entry)"
                class="text-gray-600"
              ><span class="text-xs text-gray-400 uppercase tracking-wide
                mr-1"
              >Max</span>{{ idleMax(entry) }}</span>
            </div>
          </template>
        </template>
        <template #after>
  ...
  >
    ...
    <template #buttons>
      <k-dialog-button @click="restWarning.visible = false"
      >Cancel</k-dialog-button>
      <k-dialog-button strong @click="onWearConfirmed"
      >Start anyway</k-dialog-button>
    </template>
  </k-dialog>
</template>
...
/**
 * Trailing run length of the most-recent item at the front of `sessions`
 * (newest first).
 */
...
/**
 * First item in the category that's actually pickable right now (falls back
...
/**
 * Bar fill fraction (0-1): fraction of max if set, else wraps every `target`
...
/**
 * Target marker position as a fraction of the bar. Null when max is unset
...
/**
 * Rest-remaining for whichever rest mechanic applies to this category: the
 * duration formula's rest period, or the rotation daily cap.
 */
