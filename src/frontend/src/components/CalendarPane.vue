  ...
  <div class="calendar-pane flex flex-col">
    <div class="flex items-center justify-between px-4 py-2">
      <button
        @click="prevWeek"
        class="
          border border-gray-300 rounded-lg px-4 py-1 text-gray-600 text-sm
        "
      >‹</button>
      <span
        class="text-sm font-medium whitespace-nowrap min-w-[8rem] text-center"
      >{{ formatWeekRange() }}</span>
      <button
        @click="nextWeek"
        class="
          border border-gray-300 rounded-lg px-4 py-1 text-gray-600 text-sm
      ...
      >
        ...
        >
          ...
          <span class="text-xs text-gray-500 leading-tight">
            {{ shortDuration(day.totalWearSeconds) }}
          </span>
        </div>
