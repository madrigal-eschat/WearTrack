...
describe('migration 011: rotation categories', () => {
  it(
    'adds type and consecutive_wear_days columns with correct ' + 'defaults',
    () => {
      runMigrations()
      const columns = prepare(`PRAGMA table_info(categories)`).all() as {
        name: string;
      }[]
  ...
  it(
    'existing categories default to type=duration, ' +
    ...
    () => {
      ...
      const row = prepare(
        `SELECT type, consecutive_wear_days FROM categories WHERE name = ?`,
      ).get('Migration011 Test') as {
        type: string;
