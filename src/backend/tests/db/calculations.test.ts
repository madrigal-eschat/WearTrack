...
describe('computeSessionStart', () => {
  ...
  it('inside rest period halves prev target/max', () => {
    ...
    // initial (900, 1800)
    const prev = {
      target_wear_seconds: 2000,
      max_wear_seconds: 4000,
      ended_at: 0,
      started_at: -100,
      rest_seconds: 500,
    };
  ...
  it(
    'floors target and max at initial values when halving inside rest ' +
      'period would go below',
    () => {
      // prev target/max well below initial (e.g. after heavy decay or repeated
      // halving)
      const prev = {
        target_wear_seconds: 100,
        max_wear_seconds: 200,
        ended_at: 0,
        started_at: -100,
        rest_seconds: 500,
      };
  ...
  () => {
    ...
    it(
      'applies the same floored decay to max independently, for ' +
        'categories with a maximum',
      () => {
        const prev = {
          target_wear_seconds: 4100,
          max_wear_seconds: 8200,
          ended_at: 0,
          started_at: -100,
          rest_seconds: 0,
        };
  ...
  () => {
    ...
    it(
      'adds floor(lapCount/2) * previous.target on the normal-growth ' +
        'branch',
      () => {
        // previous session: target 100, elapsed 350 (1000 - 650) =>
        // lapCount = 3, floor(3/2) = 1
        const prev = {
          target_wear_seconds: 100,
          max_wear_seconds: null,
          ended_at: 1000,
    ...
    it(
      'adds the same carry-over on the early-restart branch, scaled by ' +
        'difficulty/2',
      () => {
        // previous session: target 100, elapsed 450 (1000 - 550) =>
        // lapCount = 4, floor(4/2) = 2
        const prev = {
          target_wear_seconds: 100,
          max_wear_seconds: null,
          ended_at: 1000,
...
describe(
  'computeSessionStart — difficulty modifier now applied on the ' +
    'early-restart branch',
  () => {
    it(
      'scales the halved target by the item difficulty modifier ' + '(bug fix)',
      () => {
        const prev = {
          target_wear_seconds: 2000,
          max_wear_seconds: 4000,
...
describe('computeRest', () => {
  it(
    'elapsed * (1 + rest_weight) * rest_multiplier, floored to ' +
      'minimum_rest',
    () => {
...
describe('rotationAvailability', () => {
  ...
  it(
    'resets to all available once every active item has had a turn ' +
      'with no repeat',
    () => {
      // Newest first: A, C, B — covers all three active items before any
      // repeat.
      const result = rotationAvailability(
        [1, 2, 3],
  ...
  it(
    'a newly added item (never worn) is immediately available even ' +
      'mid-cycle',
    () => {
      // Item 4 was added after B and C were worn; it has never appeared in
      // history.
      const result = rotationAvailability(
        [1, 2, 3, 4],
        [{ item_id: 3 }, { item_id: 2 }],
  ...
  it(
    'a removed item drops out of consideration even if it was worn ' +
      'most recently',
    () => {
      // Item 3 was worn most recently but has since been removed from the
      // category (not in activeItemIds).
      const result = rotationAvailability(
        [1, 2],
        [{ item_id: 3 }, { item_id: 1 }],
      );
  ...
  it(
    'lock scenario: two consecutive sessions of the same item collapse ' +
      'to one occurrence',
    () => {
      // A worn on day1 and day2 (consecutive-wear-days lock), B and C never
      // worn.
      const result = rotationAvailability(
        [1, 2, 3],
        [{ item_id: 1 }, { item_id: 1 }],
  ...
  it(
    'does not spuriously report a full reset when a new cycle starts ' +
      "with the previous cycle's completing item interleaved further back",
    () => {
      // Chronologically: wore 1, then 2 (completes a 2-item cycle -> reset),
      // then
      // 1 again (starts a new cycle).
      // Only item 1 has been worn in the new cycle, so item 2 should be the
      // only
      // one available.
      const result = rotationAvailability(
        [1, 2],
...
describe('startOfTodayLocal / startOfNextLocalMidnight', () => {
  it(
    'startOfTodayLocal returns a timestamp at local midnight on the ' +
      'same day as `now`',
    () => {
