...
import runMigration004 from
  '../../src/db/migrations/004_drop_legacy_columns.js';
  ...
  dbExport
    .prepare(
      `INSERT INTO categories
         (name, icon, initial_wear_duration_seconds, rest_multiplier,
          rest_constant_seconds, risk_levels, break_decay_multiplier,
  ...
  dbExport
    .prepare(
      `INSERT INTO sessions
         (item_id, started_at, ended_at, calculated_wear_seconds,
...
function categoryCols(): string[] {
  return (
    dbExport.prepare('PRAGMA table_info(categories)').all() as Array<{
      name: string;
    }>
  ).map((r) => r.name);
}
function sessionCols(): string[] {
  return (
    dbExport.prepare('PRAGMA table_info(sessions)').all() as Array<{
      name: string;
    }>
  ).map((r) => r.name);
}
...
describe('migration 003 + 004', () => {
  ...
  it('backfills category values from legacy data', () => {
    ...
    // floor(1800 * 2/3)
    expect(cat.initial_target_wear_duration_seconds).toBe(1200);
