...
describe('events poller tick()', () => {
  it(
    'does not fire rest_start/decay_start on the first-ever tick for ' +
      'existing history (no backfire)',
    async () => {
  ...
  it(
    'fires target_met once for an open session, resets on a new ' + 'session',
    async () => {
      const { categoryId, itemId } = await setupCategoryAndItem();
      const startRes = await app.request(`${SESSIONS}/start`, {
        method: 'POST',
      ...
      // target_wear_seconds: 900 (first session)
      const session = await startRes.json();
      ...
      // fires again for the new session
      expect(listener).toHaveBeenCalledTimes(2);
    },
  );

  it(
    'does not fire rest_start/rest_end/halfway/decay events for a ' +
      'stale previous session while a new session is open in the same ' +
      'category',
    async () => {
      const { categoryId, itemId } = await setupCategoryAndItem();

      // Session A: ends with rest owed (rest_seconds floored at minimum_rest =
      // 86400).
      ...
      // Session B starts (and stays open) in the same category while A's
      // rest/decay clock is still ticking.
      ...
      // now=300: session A's rest window (ends ~86500) would put resting 0 ->
      // 1, and this is
      // well past A's halfway point too. With session B open, none of A's
      // previous-session
      // events should fire.
      tick(300);
  ...
  it(
    'fires rest_end once when now crosses previous.ended_at + ' +
      'previous.rest_seconds, does not refire',
    async () => {
      const { categoryId, itemId } = await setupCategoryAndItem();
      const startRes = await app.request(`${SESSIONS}/start`, {
        method: 'POST',
  ...
  it(
    'fires decay_start once when decay_state transitions from none to ' +
      'decaying, does not refire',
    async () => {
      const { categoryId, itemId } = await setupCategoryAndItem();
      const startRes = await app.request(`${SESSIONS}/start`, {
        method: 'POST',
  ...
  it(
    'fires decay_finish once when decay_state transitions to ' +
      'fully_decayed, does not refire',
    async () => {
      const { categoryId, itemId } = await setupCategoryAndItem();
      const startRes = await app.request(`${SESSIONS}/start`, {
        method: 'POST',
  ...
  it(
    'fires idle_halfway_reached once when now crosses the halfway ' +
      'point, does not refire',
    async () => {
      const { categoryId, itemId } = await setupCategoryAndItem();
      const startRes = await app.request(`${SESSIONS}/start`, {
        method: 'POST',
  ...
  it(
    'fires decay_soon once when now crosses the decay-soon fire time, ' +
      'does not refire',
    async () => {
      const { categoryId, itemId } = await setupCategoryAndItem();
      const startRes = await app.request(`${SESSIONS}/start`, {
      ...
      // decay_soon fire time = decayStart(172900) - 3600 = 169300. With
      // break_grace_time
      // 86400, this is well clear of both the rest-end+3600 and halfway
      // suppression windows.
  ...
  it(
    'fires overtime_warning_30 and overtime_warning_5 once each for an ' +
      'open session with max_wear_seconds set',
    async () => {
      const { categoryId, itemId } = await setupCategoryAndItem({
      ...
      // max_wear_seconds: 7200 (first session)
      const session = await startRes.json();
      ...
      expect(warning30.mock.calls[0][0]).toMatchObject({
        category_id: categoryId,
      ...
      expect(warning5.mock.calls[0][0]).toMatchObject({
        category_id: categoryId,
        session_id: session.id,
      });
  ...
  it(
    'fires overtime once when now crosses started_at + ' +
      'max_wear_seconds, does not refire',
    async () => {
      const { categoryId, itemId } = await setupCategoryAndItem({
        initial_max_wear_duration_seconds: 7200,
      });
  ...
  it(
    'resets halfway_notified/decay_soon_notified to 0 when a new rest ' +
      'cycle starts (resting flips 0 -> 1)',
    async () => {
      const { categoryId, itemId } = await setupCategoryAndItem();
      const startRes = await app.request(`${SESSIONS}/start`, {
        method: 'POST',
      ...
      // rest ends at 86500; seed as if halfway/decay_soon already fired for a
      // PRIOR rest cycle,
      // with `resting` recorded as 0 (i.e. the poller last observed the
      // category not resting).
      ...
      // now=150 is well inside the rest window (ends 86500): freshly computed
      // resting is 1,
      // so this tick observes a fresh 0 -> 1 transition and should reset both
      // notified flags.
      tick(150);
  ...
  it(
    'emits poller_tick once per tick(), after category processing',
    async () => {
