...
test.describe('CalendarPane week navigation', () => {
  ...
  test('calendar pane is visible on the home page', async ({ page }) => {
    // The calendar renders in a fixed-height strip at the bottom of the Home view.
    // Its prev/next navigation buttons are always present.
  ...
  test('shows a week-range label', async ({ page }) => {
    ...
    const label = page.locator('.calendar-pane .flex.items-center span.font-medium');
  ...
  test('today column is highlighted', async ({ page }) => {
    // Today's column gets the class "bg-blue-50" and its day number gets "text-blue-600".
  ...
  test('clicking previous week changes the week-range label', async ({ page }) => {
    const label = page.locator('.calendar-pane .flex.items-center span.font-medium');
    ...
    await page.waitForFunction(
      (prev) => {
        const el = document.querySelector('.calendar-pane .flex.items-center span.font-medium');
  ...
  test('clicking next week after previous week returns to the current week label', async ({
    page,
  }) => {
    const label = page.locator('.calendar-pane .flex.items-center span.font-medium');
  ...
  test('previous-week navigation shifts all displayed day numbers', async ({ page }) => {
    // Capture the first day number in the current week
    const dayNums = page.locator('.calendar-pane .grid.grid-cols-7 > div span.font-semibold');
  ...
  test('days with no sessions show a small grey dot', async ({ page }) => {
    // When totalWearSeconds === 0 the template renders a w-2 h-2 rounded-full bg-gray-200 div.
    // On a fresh DB (after global-setup reset) all days should be empty.
    // We assert at least one empty-day indicator is present.
    const emptyDots = page.locator('.calendar-pane .bg-gray-200.rounded-full');
    // There may be zero after data has been added, but on a clean state there will be some.
    // We use a soft assertion — if sessions exist, the dot may not be there.
    const count = await emptyDots.count();
    // Just verify the element type renders — count may be 0 on a seeded DB
