...
export function bandNamesForCount(count: number): string[] {
  ...
  return BAND_NAMES[count - 1]; // nosemgrep: gitlab.eslint.detect-object-injection
}
...
export function bandColorsForCount(count: number): string[] {
  ...
  return BAND_COLORS[count - 1]; // nosemgrep: gitlab.eslint.detect-object-injection
}
...
export function buildRiskLevels(bandCount: number, crossoverPoints: number[]): RiskLevel[] {
  ...
  return Array.from({ length: bandCount }, (_, i) => ({
    lower: i === 0 ? null : crossoverPoints[i - 1], // nosemgrep: gitlab.eslint.detect-object-injection
    upper: i === bandCount - 1 ? null : crossoverPoints[i], // nosemgrep: gitlab.eslint.detect-object-injection
    text: BAND_NAMES[bandCount - 1][i], // nosemgrep: gitlab.eslint.detect-object-injection
    severity: i + 1,
  }));
}
